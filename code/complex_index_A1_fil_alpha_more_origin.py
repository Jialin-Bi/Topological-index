...
def degree_heterogeneity(G):
    ...
    degree_h=degree_h/(len(G.nodes)-2*np.sqrt(len(G.nodes)-1))
    return degree_h    
...
if __name__ == "__main__":
    ...
    # for ename in range(333,527):
        print(ename)
        #file=pd.read_excel('./coordinates/origin_mutation_coodinates_'+resid1+'_'+resid2+'/'+'6M0J_'+str(ename)+'_'+resid1+'_'+resid2+'_origin'+area+'_coodinates.xlsx',index_col=0)
        file=pd.read_excel('./coordinates/'+'6M0J_'+str(ename)+'_'+resid1+'_'+resid2+'_origin'+area+'_coodinates.xlsx',index_col=0)
        ...
        File1=open('./origin_result_A1_'+resid1+'_'+resid2+ad+'/'+result_name,"w+",newline='')
        writing_File1=csv.writer(File1)       
        flag=1
         #generate simplices from distance matrix------------------------------------------------------------------------ 
        alpha_complex = gudhi.AlphaComplex(points=file_value[:,-3:])        
        for filtration_value in filtration_range:
            simplex_tree = alpha_complex.create_simplex_tree(max_alpha_square=filtration_value)
            # alpha_complex = gudhi.RipsComplex(points=file_value[:,-3:],max_edge_length=10)  
            ...
            if (ad=='_low' and simplex_dict.keys().__contains__(dimen) ) or (ad=='_c' and simplex_dict.keys().__contains__(dimen)) or (ad=='_up' and simplex_dict.keys().__contains__(dimen+1)):
                #I_1
                ...
                # print("a0")
                #A1--------------------------------------------------------------------
                upper_adjacency_matrix,lower_adjacency_matrix,simplex_adjacency_matrix=lower_uper_k_adjacency_matrix(simplex_dict,dimen)#=1
                # print("a1")
                # #A2--------------------------------------------------------------------
                ...
                if ad=='_low':
                     mat=lower_adjacency_matrix
                elif ad=='_up':
                    mat=upper_adjacency_matrix
                else:
                    mat=simplex_adjacency_matrix
                graph = nx.from_numpy_matrix(mat) 
                nodes=graph.nodes
                ...
                n=len(nodes)
                m=len(edges)
                if m>0: #graph have node  have edge 
                    ...
                    result={}
                    result['n']=n#graph have node don't have edge 
                    result['m']=0
                    result['delta']=0
            ...
            else: #graph don't have node don't have edge 
