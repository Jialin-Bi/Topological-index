...
def degree_heterogeneity(G):
    ...
    degree_h=degree_h/(len(G.nodes)-2*np.sqrt(len(G.nodes)-1))#标准化
    return degree_h    
...
if __name__ == "__main__":
    ...
    for ename in file_name[:1]:
        ...
        file_col=file.columns#read coodinates
        file_index=file.index
        file_value=file.values

        result_name=ename+'_'+resid1+'_'+resid2+area+dim+'index.csv'
        File1=open('./mutation_result'+dim+resid1+'_'+resid2+ad+'/'+result_name,"w+",newline='')
        ...
        for filtration_value in filtration_range:
            ...
            if (ad=='_low' and simplex_dict.keys().__contains__(dimen) ) or (ad=='_c' and simplex_dict.keys().__contains__(dimen)) or (ad=='_up' and simplex_dict.keys().__contains__(dimen+1)):
                ...
                if m>0: #graph have node  have edge 
                    ...
                    #average communicability  & average communicabolity angle#nx.communicability(G1)
